...
*
* Revised 2021/2022 by Blaise & Bruno: 
* Added: filter by conversion name   
* Added: gradient formatting for key performance indicators
* Added: freeze header columns for a better readability  
* Revised: allow to run the script without any campaign name as parameter
* Added 2022-01: filter by advertising channel
* Upgraded 2022-11: integrating AWQL migration by Nils Rooijmans
**/
function main() {
//////////////////////////////////////////////////////////////////////////////
// Options
...
// Use this if you only want to look at some campaigns with names containing 'FRANCE' or leave blank
var campaignNameDoesNotContain = "";
// Use this if you want to exclude some campaigns
// such as campaigns with names containing 'Brand' or leave blank 
// Filter by Conversion type name  
var ConversionTypeName = ""; // Filter by conversion type (B&B)
  
var advertisingChannelType = "";  // Filter by advertising channel eg. SEARCH or SHOPPING or leave blank (B&B)  
...
var conversionThreshold = 0;
  ...
  if (advertisingChannelType != '') {
    whereStatements += "AND AdvertisingChannelType = " + advertisingChannelType + " " ;
  }
  ...
  // Revised B&B: allow script to execute with no campaigns parameters
  var campaignReport = ''; 
  if (campaignNameContains){
      campaignReport = AdWordsApp.report(
  ...
  // ////////////////////////////////////////////////////////////////////////////
  // Retrieve keywords ids for specific conversion types -- Blaise & Bruno 6/2021 
  
  if (ConversionTypeName) { 
   var queryConvReport = AdWordsApp.report(
     "SELECT KeywordId " +
     "FROM SEARCH_QUERY_PERFORMANCE_REPORT " +
      "WHERE  ConversionTypeName CONTAINS_IGNORE_CASE '"+ ConversionTypeName + "' AND CampaignId IN [" + activeCampaignIds.join(",") + "] " +       
     "DURING " + dateRange);
  
    var queryConvRows = queryConvReport.rows();
    var convKeywordsIds = [] ;
  
    while (queryConvRows.hasNext()) {
      var queryConvRow = queryConvRows.next();
      convKeywordsIds.push(queryConvRow["KeywordId"]);
    }
} 
  ...
  var queryReport = AdWordsApp.report(
    'SELECT CampaignName, CampaignId, AdGroupId, AdGroupName, KeywordId, Query, ' + statColumns.join(', ') + ' ' // Added B&B: KeywordId
    + 'FROM SEARCH_QUERY_PERFORMANCE_REPORT '
  ...
  while (queryRows.hasNext()) {
    var queryRow = queryRows.next();
      ...
      if (ConversionTypeName) {// B&B: Check if Keyword conv. type should be taken
   
        var currentKeywordId = queryRow["KeywordId"];
        if (!convKeywordsIds.includes(currentKeywordId)){
          {continue;}
        }  
      }    
  ...
  // Filter by conversion type (Added B&B)
  if (ConversionTypeName){
    filterText += " filtered by conversion type containing '" + ConversionTypeName + "'";
  }
  ...
  for (var i = 0; i < 5; i++) {
    try {
      ...
      if (formatArray.length == 0) {
        ...
        // We prefer to sort by Conversions then Cost per conv.  (B&B)
        sortByColumns.push({ column: outputArray[0].indexOf('Conversions') + 1, ascending: false });   
        sortByColumns.push({ column: outputArray[0].indexOf('Cost / conv.') + 1, ascending: false });  
      ...
      // Gradient formatting -- Added B&B
      var range = sheet.getRange(lastRow + 2, outputArray[0].indexOf('Cost') + 1, lastRow + outputArray.length);      
      var rule = SpreadsheetApp.newConditionalFormatRule()
                                .setGradientMaxpoint("#ffd666").setGradientMinpoint("#ffffff")
                                .setRanges([range])
                                .build()
      ...
      var range = sheet.getRange(lastRow + 2, outputArray[0].indexOf('Conversions') + 1, lastRow + outputArray.length);      
      var rule = SpreadsheetApp.newConditionalFormatRule()
      ...
      var range = sheet.getRange(lastRow + 2, outputArray[0].indexOf('Cost / conv.') + 1, lastRow + outputArray.length);      
      var rule = SpreadsheetApp.newConditionalFormatRule()
                                .setGradientMaxpoint("#e67c73").setGradientMinpoint("#57bb8a")
                                .setRanges([range])
                                .build()
      ...
      rules.push(rule);
      sheet.setConditionalFormatRules(rules); 
      sheet.setFrozenRows(4);  // Freeze header rows for better readability (B&B)
